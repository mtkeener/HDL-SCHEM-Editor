""" Handles all stuff for inserting an input connector into the canvas"""
import interface_insertion

class Input(interface_insertion.InterfaceInsertion):
    # There is a "dummy" entry to keep the number of parameters of Input() identical to the number of parameters of InterfaceInsertion().
    def __init__(self, schematic_window, diagram_tab, dummy, follow_mouse, #push_design_to_stack=True,
        ...
        lower_right_corner = [-1/3, +1/6 ]
        lower_left_corner  = [-3/3, +1/6 ]
        upper_left_corner  = [-3/3, -1/6 ]
        upper_right_corner = [-1/3, -1/6 ]
        base_corner1       = [-1/6, -1/30]
        point_corner1      = [   0, -1/30]
        points = [
            anchor,  # Must be the first point in the list.
